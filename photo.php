...
try {
    ...
    if (isset($_GET['del'])) {
        $sql = "DELETE FROM travelimage WHERE UID = '".$_COOKIE['Username']."' AND ImageID = '".$_GET['del']."'";
        $pdo->query($sql);
    }
    $sql = "SELECT ImageID,Title,Description,PATH FROM travelimage WHERE UID = '".$_COOKIE["Username"]."'";
...
<div class="con">
    ...
    function makeImg($path, $des, $title, $id) {
        ...
        echo "<p>".$des."</p>";
        echo "<button class='black'><a href=\"upload.php?id=".$id."\">
                 <img src=\"images/sup/modify.png\">
                 <span>Modify</span>
               </a></button>";
        echo "<button class='red'><a href=\"photo.php?del=".$id."\">
